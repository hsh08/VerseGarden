...
import { loadBibleRepository } from "@/features/bible/bibleRepository";
import { qtRepository } from "@/features/qt/qtRepository";
import { dateKeyFor, type QTContentPreview } from "@/features/qt/qtTypes";
...
export default function HomeTab() {
  ...
  const [todayQTPreview, setTodayQTPreview] = useState<QTContentPreview | null>(null);
  ...
  const todayQT = qtRecord(dateKeyFor(new Date(), selectedCommunity?.timezone));
  useEffect(() => { traceStartup("Home mounted"); }, []);
  useEffect(() => {
    let active = true;
    void qtRepository.resolveTodayPreview(new Date(), selectedCommunity).then((preview) => { if (active) setTodayQTPreview(preview); }).catch(() => undefined);
    return () => { active = false; };
  }, [selectedCommunity]);
  useEffect(() => {
    if (!__DEV__ || !todayQT?.completedAt) return;
    console.info("[QT Complete] home completion observed", { source: todayQT.contentSource ?? "unknown" });
  }, [todayQT?.completedAt, todayQT?.contentSource]);
  const openWriting = () => {
    if (!selected || !next) return;
    void loadBibleRepository().then((repository) => {
  ...
  return <VGScreen>
    ...
    {todayQT?.completedAt ? <VGCard tinted style={styles.qtCard}><Text style={styles.eyebrow}>{todayQTPreview?.source === "community" ? `공동체 QT · ${todayQTPreview.communityName ?? ""}` : "오늘의 QT"}</Text><Text style={styles.planTitle}>{todayQTPreview?.source === "community" ? "공동체 QT 완료" : "오늘의 QT 완료"}</Text><Text style={styles.planRange}>오늘 남긴 기록을 다시 확인할 수 있어요.</Text><Pressable accessibilityRole="button" onPress={() => router.push("/qt" as never)} style={({ pressed }) => [styles.detailCta, pressed && styles.pressed]}><Text style={styles.detailCtaLabel}>QT 기록 보기</Text></Pressable></VGCard> : <VGCard tinted style={styles.qtCard}><Text style={styles.eyebrow}>{todayQTPreview?.source === "community" ? `공동체 QT · ${todayQTPreview.communityName ?? ""}` : "오늘의 QT"}</Text><Text style={styles.planTitle}>{todayQT ? "오늘의 QT 이어하기" : todayQTPreview?.title ?? "오늘의 QT 시작하기"}</Text><Text style={styles.planRange}>{todayQTPreview?.source === "community" ? "공동체와 함께 오늘의 말씀을 묵상해보세요." : todayQTPreview?.devotionalText ?? "말씀을 읽고 묵상과 기도를 남겨보세요."}</Text><Pressable accessibilityRole="button" onPress={() => router.push("/qt" as never)} style={({ pressed }) => [styles.cta, pressed && styles.pressed]}><Text style={styles.ctaLabel}>{todayQT ? "QT 이어하기" : "QT 시작하기"}</Text></Pressable></VGCard>}
