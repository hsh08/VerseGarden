...
export default function GardenTab() {
  const { activities, stats } = useGarden();
  const [displayedMonth, setDisplayedMonth] = useState(() => dateForKey(`${dateKeyFor().slice(0, 7)}-01`));
  const [selectedDateKey, setSelectedDateKey] = useState<string | null>(dateKeyFor());
  ...
  const selectedActivities = useMemo(() => selectedDateKey ? activities.filter((activity) => dateKeyFor(activity.createdAt) === selectedDateKey) : [], [activities, selectedDateKey]);
  const selectedSummary = selectedDateKey ? stats.summariesByDateKey[selectedDateKey] ?? null : null;

  const selectDate = (dateKey: string) => {
    const nextDateKey = selectedDateKey === dateKey ? null : dateKey;
    setSelectedDateKey(nextDateKey);
    if (__DEV__) console.info("[Garden Date]", { activities: nextDateKey ? activities.filter((activity) => dateKeyFor(activity.createdAt) === nextDateKey).length : 0, selected: nextDateKey });
  };
  ...
  return <VGScreen>
    ...
    <VGCard style={styles.heatmapCard}>
      ...
      <View style={styles.grid}>{cells.map((dateKey, index) => dateKey ? <Pressable accessibilityLabel={`${dateKey} 기록 보기`} accessibilityRole="button" accessibilityState={{ selected: selectedDateKey === dateKey }} key={dateKey} onPress={() => selectDate(dateKey)} style={({ pressed }) => [styles.dayCell, stats.summariesByDateKey[dateKey]?.hasGrowthActivity && styles.activeDay, dateKey === todayKey && styles.todayDay, selectedDateKey === dateKey && styles.selectedDay, pressed && styles.pressed]}><Text style={[styles.dayLabel, stats.summariesByDateKey[dateKey]?.hasGrowthActivity && styles.activeDayLabel, selectedDateKey === dateKey && styles.selectedDayLabel]}>{dateKey.slice(-2).replace(/^0/, "")}</Text><Text style={[styles.dayCount, stats.summariesByDateKey[dateKey]?.hasGrowthActivity && styles.activeDayLabel, selectedDateKey === dateKey && styles.selectedDayLabel]}>{stats.summariesByDateKey[dateKey]?.totalGrowthActivityCount ?? ""}</Text></Pressable> : <View key={`blank-${index}`} style={styles.dayCell} />)}</View>
      <Text style={styles.legend}>색이 있는 날짜는 말씀 저장, 필사, 기도, QT 완료 기록이 있는 날입니다.</Text>
    </VGCard>
    {selectedDateKey ? <VGCard tinted style={styles.todayCard}><Text style={styles.todayTitle}>{selectedDateKey === todayKey ? "오늘의 기록" : `${selectedDateKey.replaceAll("-", ".")} 기록`}</Text><Text style={styles.todayText}>{selectedSummary?.hasGrowthActivity ? `말씀 저장 ${selectedSummary.verseLikedCount} · 필사 ${selectedSummary.scriptureCopyCount} · 기도 ${selectedSummary.prayerCount} · QT ${selectedSummary.qtCompletedCount}` : "이 날 남긴 성장 기록이 아직 없어요."}</Text>{selectedActivities.length ? <View style={styles.selectedList}>{selectedActivities.map((activity) => <View key={activity.id} style={styles.selectedActivity}><View><Text style={styles.activityTitle}>{gardenActivityLabels[activity.type]}</Text><Text style={styles.activityMeta}>{activity.reference ?? activity.title}</Text></View><Text style={styles.activityDate}>{dateKeyFor(activity.createdAt)}</Text></View>)}</View> : <Text style={styles.emptyDay}>이 날 저장된 활동 기록이 없습니다.</Text>}</VGCard> : null}
...
const styles = StyleSheet.create({
  ...
  weekdays: { flexDirection: "row" }, weekday: { color: colors.secondaryText, flex: 1, textAlign: "center", ...typography.caption }, grid: { flexDirection: "row", flexWrap: "wrap", rowGap: spacing.xs }, dayCell: { alignItems: "center", borderRadius: radius.small, height: 46, justifyContent: "center", width: "14.2857%" }, activeDay: { backgroundColor: colors.sageGreen }, todayDay: { borderColor: colors.forestGreen, borderWidth: 1 }, selectedDay: { backgroundColor: colors.forestGreen, borderColor: colors.forestGreen, borderWidth: 1 }, dayLabel: { color: colors.secondaryText, ...typography.caption }, dayCount: { color: colors.secondaryText, fontSize: 11, lineHeight: 14 }, activeDayLabel: { color: colors.cardBackground, fontWeight: "700" }, selectedDayLabel: { color: colors.cardBackground, fontWeight: "700" }, pressed: { opacity: 0.72 }, legend: { color: colors.secondaryText, ...typography.caption },
  todayCard: { gap: spacing.xs }, todayTitle: { color: colors.primaryText, ...typography.heading }, todayText: { color: colors.secondaryText, ...typography.body }, selectedList: { borderTopColor: colors.border, borderTopWidth: StyleSheet.hairlineWidth, gap: spacing.sm, marginTop: spacing.xs, paddingTop: spacing.sm }, selectedActivity: { alignItems: "center", flexDirection: "row", justifyContent: "space-between" }, emptyDay: { color: colors.subtleText, ...typography.caption }, recentList: { gap: spacing.sm }, activityCard: { alignItems: "center", flexDirection: "row", justifyContent: "space-between", padding: spacing.md }, activityTitle: { color: colors.primaryText, ...typography.bodyEmphasis }, activityMeta: { color: colors.secondaryText, ...typography.caption }, activityDate: { color: colors.subtleText, ...typography.caption },
});
